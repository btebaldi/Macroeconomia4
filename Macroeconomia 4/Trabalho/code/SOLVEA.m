function [ind,dropm,A] = solvea(alphas,qq,ne)
...
if rank(alphas(:,1:n)) == 0
   ...
   %**********************temporary lines, to be deleted when no longer necessary***********
%    disp('(solvea) maximal eigenvalue less than unity')
%    rrt(II)
%    abs(rrt(II))
%    disp('(solvea) absolute value of all eigenvalues')
%    abs(rrt)
   %****************************************************************************************
