...
% ATENCAO ESSE SCRIPT PODE TER DIVERGENCIAS DO SCRIPT APRESENTADO NO REPORT
...
% The mean
Means = [mean(PolicyFunction.c(:)) mean(PolicyFunction.h(:)) mean(PolicyFunction.k1(:)) mean(PolicyFunction.i(:))];
Vars = [var(PolicyFunction.c(:)) var(PolicyFunction.h(:)) var(PolicyFunction.k1(:)) var(PolicyFunction.i(:))];
disp(table(categorical({'Consumption'; 'Hours worked'; 'Capital'; 'Investment'}),Means',Vars', 'VariableNames',{'Variable' 'Mean' 'Variance'}));